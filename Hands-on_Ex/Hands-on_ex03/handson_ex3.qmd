...
First, we write code chunk to install and launch the following R packages:
...
[ggiraph](https://davidgohel.github.io/ggiraph/index.html) ![](https://r4va.netlify.app/chap03/img/image1.jpg){width="46"} is an htmlwidget and a ggplot2 extension. It allows ggplot graphics to be interactive.
...
Code chunk below shows an advanced way to customize tooltip. In this example, a function is used to compute 90% confident interval of the mean. The derived statistics are then displayed in the tooltip.
...
The code chunk below shown an example of `onclick`. Note that click actions must be a string column in the dataset containing valid javascript instructions.
...
Plotly’s R graphing library create interactive web graphics from **ggplot2** graphs and/or a custom interface to the (MIT-licensed) JavaScript library [**plotly.js**](https://plotly.com/javascript/) inspired by the grammar of graphics. Different from other plotly platform, plot.R is free and open source. ![](images/clipboard-644065865.png)
...
The chunk below shows an example a basic interactive plot created by using *plot_ly()*.
...
-   Kam, T. S. (2024). *Programming Interactive Data Visualisation with R.* <https://r4va.netlify.app/chap03>
